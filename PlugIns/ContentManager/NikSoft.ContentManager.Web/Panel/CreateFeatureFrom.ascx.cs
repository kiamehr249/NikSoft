...
{
    public partial class CreateFeatureFrom : NikUserControl
    {
        ...
        {
            var categories = iContentCategoryServ.GetAll(x => x.PortalID == PortalUser.PortalID && x.Enabled);
            ddlCategory.FillControl(categories.Select(t => new { t.ID, t.Title }).ToList(), "Title", "ID");
        }
