<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="rd_ContentCategory.ascx.cs" Inherits="NikSoft.ContentManager.Web.Panel.rd_ContentCategory" %>
<Nik:ModalSearch runat="server">
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="form-group">
                <label class="control-label" for="ddlGroup">گروه محتوایی</label>
...
<div class="row">
    <div class="col-md-12 table-responsive">
        <Nik:NikGridView ID="GV1" PageSize="15" runat="server" ItemType="NikSoft.ContentManager.Service.ContentCategory" OnRowCommand="GV1_RowCommand">
            <EmptyDataTemplate>
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="دسته بندی بالاتر">
                    <ItemTemplate>
                        <%# Item.Parent != null ? Item.Parent.Title : "ندارد" %>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="کلید">
                    <ItemTemplate>
                        <%# Item.ModuleKey != null ? Item.ModuleKey : "ندارد" %>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="تصویر">
                    <ItemTemplate>
                        <img src='<%# "/" + Item.ImgUrl %>' class="grid-item-img" style="max-width: 70px;" />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="دسته های زیرین">
                    <ItemTemplate>
                        <a href="<%# "/panel/rd_ContentCategory/" + Item.ID %>" class="btn btn-default btn-edite">زیر دسته ها</a>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="ویژگی ها">
                    <ItemTemplate>
                        <asp:HyperLink ID="HypFeatures" runat="server" NavigateUrl='<%# "/panel/ContentFeatures/" + Item.ID %>' CssClass="btn btn-default btn-edite" Visible="<%# Item.HasFeature && Item.Childs.Count < 1 %>">ویژگی ها</asp:HyperLink>
                    </ItemTemplate>
                </asp:TemplateField>
...
<script type="text/javascript">
    $(document).ready(function () {
        FirstOnChange('#ddlTitle', '#ddlGroup', GetContentGroup);
		<%= SelectData %>

        function GetContentGroup(gID, jID) {
            var ddlGroups = $('#ddlGroup');
            var data = "{'TitleID':'" + gID + "'}";
            var url = '<%=ResolveUrl("../WebService/ContentWebService.asmx/GetContentGroup") %>';
