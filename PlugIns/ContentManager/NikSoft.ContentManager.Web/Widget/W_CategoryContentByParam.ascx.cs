...
{
    public partial class W_CategoryContentByParam : WidgetUIContainer
    {
        ...
        public void Dataload()
        {
            ...
            {
                int catid = ModuleParameters.ToInt32();
                var thisCont = iContentCategoryServ.Find(x => x.ID == catid, y => new { y.ID, y.GroupID });
                if (lastorder)
                    ContentList = iPublicContentServ.GetAll(x => x.PortalID == CurrentPortalID && x.Enabled && x.GroupID == thisCont.GroupID && x.CategoryID != thisCont.ID).Take(countOfData).OrderByDescending(x => x.ID).ToList();
                else
                    ContentList = iPublicContentServ.GetAll(x => x.PortalID == CurrentPortalID && x.Enabled && x.GroupID == thisCont.GroupID && x.CategoryID != thisCont.ID).Take(countOfData).OrderBy(x => x.Ordering).ToList();
            }
            else
            {
                var thisCont = iContentCategoryServ.Find(x => x.ModuleKey == ModuleParameters , y => new { y.ID, y.GroupID });
                if (lastorder)
                    ContentList = iPublicContentServ.GetAll(x => x.PortalID == CurrentPortalID && x.Enabled && x.GroupID == thisCont.GroupID && x.CategoryID != thisCont.ID).Take(countOfData).OrderByDescending(x => x.ID).ToList();
                else
                    ContentList = iPublicContentServ.GetAll(x => x.PortalID == CurrentPortalID && x.Enabled && x.GroupID == thisCont.GroupID && x.CategoryID != thisCont.ID).Take(countOfData).OrderBy(x => x.Ordering).ToList();
